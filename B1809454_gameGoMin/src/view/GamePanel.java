				...
				if(e.getButton() == 1 && e.getSource() == arrayButton[i][j] && !world.getArrayCamCo()[i][j]) {
					...
					if(!world.open(i,j)) {
						...
						if(world.isComplete()) {
							...
							getP1().getBt().repaint();
							int option = JOptionPane.showConfirmDialog(this, "Bạn đã thua, có muốn chơi lại không?", "Notification", JOptionPane.YES_NO_OPTION);
							...
							getP1().getBt().repaint();
							int option = JOptionPane.showConfirmDialog(this, "Bạn đã chiến thắng, có muốn chơi lại không?", "Notification", JOptionPane.YES_NO_OPTION);
							if(option == JOptionPane.YES_OPTION) {
								gameFrame.setVisible(false);
								new GameFrame(w, h ,boom);
							}
