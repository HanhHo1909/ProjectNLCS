...
public class LabelNumber extends JLabel{
	...
	public void paint(Graphics g) {
		if(number.equals("voCuc")) {
			g.drawImage(p.getGame().getGameFrame().getLoadData().getListImage().get("voCung"), 0, 0, 26, 46, null);
			g.drawImage(p.getGame().getGameFrame().getLoadData().getListImage().get("voCung"), 26, 0, 26, 46, null);
			g.drawImage(p.getGame().getGameFrame().getLoadData().getListImage().get("voCung"), 52, 0, 26, 46, null);
		}else {
